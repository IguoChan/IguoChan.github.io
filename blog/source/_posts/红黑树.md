...
但是有一点区别就是，需要记住真正被删除节点的颜色，并记住顶替被删除节点的节点，这是因为后续进行再平衡时需要，基本代码如下（暂时忽略origin和rbtree_delete_fixup等）：
``` c
...
所以我们的修复方案是记住真实被删除节点的颜色（使用tmp的颜色）和代替其位置的节点（origin），然后依此进行判断。这个时候将节点origin（也就是图中的current，图片借鉴[红黑树原理 新增，删除，打印](https://www.cnblogs.com/hongdada/p/12165636.html)）看成是双重颜色：双黑色（origin和被删除的节点都是黑色），红黑色（oringin是红色的，但是被删除节点是黑色的）。需要修正的情况有以下4种case。以下情况都是代替节点是其父节点左儿子的情况，右儿子是对称的。循环条件就是代替节点origin不是根节点且其颜色是黑色的，如果是红色或者根节点，直接涂黑就好了。循环操作的目标是将额外的黑色性质上移，循环的节点首先就是origin，其后处理可能会变。
